class SaveReload(object):
    def __init__(save_self, database="working_database.csv"):
        ...
        save_self.select_new_file_box = wg.Text()
        save_self.select_old_file_box = wg.Text(value=database)
        ...
        save_self.vbox = wg.VBox([wg.HBox([wg.Label("Start new data file"), save_self.select_new_file_box]),
                                    wg.HBox([wg.Label("Add to existing data file"), save_self.select_old_file_box]),
...
#         print("Triggered SaveReload.on_confirm_save_location_clicked")
        ...
        if len(save_self.select_new_file_box.value) > 0:
            save_self.save_path = save_self.select_new_file_box.value
            setattr(save_self, "_reload", False)
            save_self.save_label.value = "Saving new data to "+str(save_self.save_path)

        elif len(save_self.select_old_file_box.value) > 0:
            save_self.save_path = save_self.select_old_file_box.value
